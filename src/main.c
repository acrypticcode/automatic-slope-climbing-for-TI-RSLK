...
float proportionalSpeedLeft = 0;
float proportionalSpeedRight = 0;
float vmax = 7.45;
...
int main(void)
{
    ...
    while(1){
        ...
        if(run_control){    // If 100 ms has passed
            ...
            acc_ary[2] = acc_ary[3];
            acc_ary[3] = acc_ary[4];
            acc_ary[4] = readAccel_y();
            readY = (acc_ary[0] + acc_ary[1] + acc_ary[2] + acc_ary[3] + acc_ary[4])/5;
            angle = 1.0*readY/(980)*(180/3.14159);


            desiredSpeed = kP * readY;
...
int16_t readAccel_y(void){
    ...
    I2C_readData(EUSCI_B1_BASE, 0x60, 14, accy, 2 );

    acc_datay = accy[0];
    acc_datay = acc_datay << 8;
